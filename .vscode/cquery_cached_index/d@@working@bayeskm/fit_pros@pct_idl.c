...
/*****************************************************************************/
/* Local functions */
double pctFunc(int parNr, double *p, void*);
/*****************************************************************************/
 ...
 *  Main
 */
int pCT_idl(int argc, char **argv)
{
  ...
  unsigned int    frameNr, isweight = 0, 
                  bootstrapIter, ri =0, inputtype=0;
  // int          fVb = -1.0;
  ...
  */
  ...
  doSD     = *(unsigned int*) argv[10]; 
  doCL     = *(unsigned int*) argv[11]; 
  bootstrapIter = *(unsigned int*) argv[12]; 
  bmatrix   = (double*) argv[13];
...
//   /* Vb    */ def_pmin[3]=0.0;       def_pmax[3]=0.08;
 ...
 /* Check that these are ok */
  ...
  for (int i=0; i<frameNr; i++) { 
    // data.x1[i] = t0[i];
    // data.x2[i] = t1[i];
    data.x[i]=t0[i]; //0.5*(data.x1[i]+data.x2[i]);
    data.voi[ri].y[i]= tac[i];

    // input.x1[i] = t0[i];
    // input.x2[i] = t1[i];
    input.x[i]=t0[i];  //0.5*(input.x1[i]+input.x2[i]);
    input.voi[ri].y[i]= ctt[i];

    if(data.isweight) { data.w[i]=weights[i]; input.w[i]=weights[i];    }
    if(!data.isweight) { data.w[i]=1.0;  input.w[i]=1.0; }
}
// printf("ctemeas %f %f %f %f\n", data.voi[ri].y[0], data.voi[ri].y[100], data.voi[ri].y[200], data.voi[ri].y[300]);
// printf("ctemeas %f %f %f %f %d\n", tac[0], tac[100], tac[200], tac[300], frameNr);
...
/*
 ...
 /*
  ...
  for(ri=0; ri<data.voiNr; ri++) if(data.voi[ri].sw==0) {
    ...
    /* Initiate values */
    ctemeas=data.voi[ri].y; ctsim=data.voi[ri].y2;
    ...
    if(doBootstrap) {
      ...
      ctemeas=data.voi[data.voiNr].y2; ctsim=data.voi[data.voiNr].y3;
      ...
      ret=bootstrapr(
        ...
        // tissue TAC noisy data is written to be used by objf
        ctemeas, 
        parNr, data.w, pctFunc, tmp, verbose-4,bmatrix
      );
 ...
 *
 *****************************************************************************/
double pctFunc(int parNr, double *p, void *fdata)
{
  int fi, ret;
  double Vb, d, wss=0.0;
  ...
  //                     ctsim, NULL);
  //   else
  //     ret=integrate(data.x, input.voi[0].y2, data.frameNr, ctsim);
  //   ret=interpolate4pet(
  //     input.x, input.voi[0].y2, input.frameNr,
  //     data.x1, data.x2, ctsim, NULL, NULL, fitframeNr);
  // else
  //   ret=interpolate(
  //     input.x, input.voi[0].y2, input.frameNr,
  //     data.x, ctsim, NULL, NULL, fitframeNr);
  // if(ret) {
  ...
  for(fi=0, wss=0.0; fi<fitframeNr; fi++) if(data.w[fi]>0.0) {
    d=ctemeas[fi]-ctsim[fi]; 
    wss+=data.w[fi]*d*d;
    // printf("test! %f %f",ctemeas[fi],ctsim[fi]);
  }
  wss_wo_penalty=wss;
  wss*=penalty;
  if(0) printf("CBF=%g  MTT=%g  => %g\n",
    pa[0], pa[1], wss);
