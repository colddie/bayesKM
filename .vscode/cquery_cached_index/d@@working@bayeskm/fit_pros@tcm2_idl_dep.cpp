 ...
 *  Main
 */
extern "C" int TCM2_IDL::tcm2_idl(int argc, char **argv)
{
...
if(verbose>0) {printf( "\n");   resPrint(&res);}
  output[0] = res.voi[0].parameter[0];     // see below
  output[1] = res.voi[0].parameter[1];
  output[2] = res.voi[0].parameter[2];
  output[3] = res.voi[0].parameter[3];
  output[4] = res.voi[0].parameter[4];
  output[5] = res.voi[0].parameter[7];
  output[6] = res.voi[0].parameter[8];
 ...
 *
 *****************************************************************************/
double TCM2_IDL::cm3Func(int parNr, double *p, void *fdata)
{
  int fi, ret;
