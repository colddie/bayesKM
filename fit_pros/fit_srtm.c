/** @file fit_srtm.c
 *  @brief NLLSQ fitting of the parameters of simplified reference tissue compartmental model 
 *   to PET TTACs.
 *  @remark This TPCCLIB version is based on previous fit_srtm version 3.1.0 / 2013-06-26. 
...
/*****************************************************************************/
static char *info[] = {
  "NLLSQ estimation of R1 (=K1/K1'), k2 and, BPnd (binding potential) using",
  "simplified reference tissue compartment model, SRTM (1,2,3).",
  "Assumptions are that K1/k2 is the same in all brain regions, and that ",
  "1TCM with plasma input could fit the tissue curves satisfactorily.",
  " ",
  ...
  " ",
  "See also: bfmsrtm, dftweigh, rescoll, logan, fit_frtm, sim_rtcm",
  " ",
...
int main(int argc, char **argv)
{
  ...
  if(limfile[0] && !ttacfile[0]) {
    ...
    iftPutDouble(&ift, "BP_lower", def_pmin[2], NULL);
    iftPutDouble(&ift, "BP_upper", def_pmax[2], NULL);
  ...
  if(limfile[0]) {
    ...
    if(iftGetDoubleValue(&ift, 0, "BP_lower", &v)>=0) {def_pmin[2]=v; n++;}
    if(iftGetDoubleValue(&ift, 0, "BP_upper", &v)>=0) {def_pmax[2]=v; n++;}
    iftEmpty(&ift);
  ...
  for(int ri=0; ri<dft.voiNr; ri++) if(ri!=ref) {
    ...
    if(doBootstrap) {
      ...
      ret=bootstrap(
        ...
        tis, 
        parNr, w, srtmFunc, tmp, verbose-5
      );
      if(ret) {
  ...
  if(doDVR!=0) {
    ...
    for(int ri=0; ri<res.voiNr; ri++) {
      res.voi[ri].parameter[2]+=1.0;
      if(doCL && !isnan(res.voi[ri].cl1[2])) res.voi[ri].cl1[2]+=1.0;
      if(doCL && !isnan(res.voi[ri].cl2[2])) res.voi[ri].cl2[2]+=1.0;
    }
  }
 ...
 *
 *****************************************************************************/
double srtmFunc(int parNr, double *p, void *fdata)
{
  int ret;
